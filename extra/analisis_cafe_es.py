...
def ttests_apareados(df: pd.DataFrame) -> pd.DataFrame:
    cafes = sorted(df["tipo_cafe"].dropna().unique().tolist())
    pares = list(itertools.combinations(cafes, 2))
    resultados = []
    # pivote por participante para asegurar pareo
    for atr in ATRIBUTOS:
        piv = df.pivot_table(index="participante_id", columns="tipo_cafe", values=atr, aggfunc="first")
        for a, b in pares:
...
def main():
    ...
    print("\n[DEBUG] Ejecutando t-tests apareados...")
    ttests = ttests_apareados(df)
    if ttests.empty:
